...
from services.ai_service import AISummaryService
...
async def get_daily_summary(date: Optional[str] = None):
    """일일 뉴스 요약 조회"""
    result = await AISummaryService.get_daily_summary(date)
...
async def create_daily_summary(
    ...
    result = await AISummaryService.generate_daily_summary(date)
